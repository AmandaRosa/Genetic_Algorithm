...
%% Média Individuos e Melhor Vetor

Media_geracao(1,1) = sum(Resultado)/Individuos;

Vetor_geracao = sort(Resultado);
Melhor_index_geracao(1,1) = Vetor_geracao(1,1);
% Melhor_index2_geracao = min(find(Melhor_index_geracao == Resultado));
% Melhor_vetor_geracao(1,:) = Geracao_inicial(Melhor_index2_geracao,:);
...
%% LOOP
    ...
    Media_geracao(loop,1) = sum(Resultado)/Individuos;
    Vetor_geracao = sort(Resultado);
    Melhor_index_geracao(loop,1) = Vetor_geracao(1,1);
%     Melhor_index2_geracao(loop,:) = min(find(Melhor_index_geracao == Resultado));
%     Melhor_vetor_geracao(loop,:) = Geracao(Melhor_index2_geracao,:);
